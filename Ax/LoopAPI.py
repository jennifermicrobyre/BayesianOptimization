...
#
# The loop API is the most lightweight way to do optimization in Ax. The user makes one call to `optimize`, which performs all of
# the optimization under the hood and returns the optimized parameters.
...
# in which case Ax will assume that evaluation corresponds to the objective. It can also return only the mean as a float, in which
# case Ax will treat SEM as unknown and use a model that can infer it. For more details on evaluation function, refer to the
# "Trial Evaluation" section in the docs.

# ## 2. Run optimization
# The setup for the loop is fully compatible with JSON. The optimization algorithm is selected based on the properties of the problem
# search space.
...
fig = plot_contour_plotly(model=model, param_x='x1', param_y='x2', metric_name='hartmann6')
fig.show()
