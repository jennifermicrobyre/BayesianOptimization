...
def inputs():
    ...
    return device, dtype, SMOKE_TEST, bounds, NOISE_SE, BATCH_SIZE, NUM_RESTARTS, RAW_SAMPLES, MC_SAMPLES, initial_data_points, dimensions, train_obj_location, train_x_location, train_x_all_location

def load_obj_make_tensor_and_save(train_obj_location):
    ...
    train_obj_all = train_obj.detach().clone()

    torch.save(train_obj, train_obj_location[:-3] + 'pt')
    torch.save(train_obj_all, train_obj_location[:-4] + '_all.pt')
...
def main():
    device, dtype, SMOKE_TEST, bounds, NOISE_SE, BATCH_SIZE, NUM_RESTARTS, RAW_SAMPLES, MC_SAMPLES, initial_data_points, dimensions, train_obj_location, train_x_location, train_x_all_location = inputs()
    train_obj, train_obj_all = load_obj_make_tensor_and_save(train_obj_location)
